...
func (f finder) FindMatchesStats7Days(playerId string) []models.Match {
	...
	curl := exec.Command("curl", "--header", "Host:api.faceit.com", "--header", "Authorization:'Bearer 3a839030-b132-4733-8815-da9a478e835b'", apiurls_constants.GetMatchList1+playerId+apiurls_constants.GetMatchList2+"20")
	out, err := curl.Output()
	if err != nil {
...
func (f finder) FindMatch(matchId string) models.Match {
	...
	// 	log.Fatal(err)
	// }
	curl := exec.Command("curl", "--header", "Host:api.faceit.com", "--header", "Authorization:'Bearer 3a839030-b132-4733-8815-da9a478e835b'", apiurls_constants.GetMatchStats+matchId)
	out, err := curl.Output()
	if err != nil {
		fmt.Println("erorr", err)
	}
...
func (f finder) FindLiveMatch(matchId string) models.LiveMatch {
	...
	// req.Header.Add("Authorization", "Bearer 3a839030-b132-4733-8815-da9a478e835b")

	// response, err := http.DefaultClient.Do(req)

	curl := exec.Command("curl", "--header", "Host:api.faceit.com", "--header", "Authorization:'Bearer 3a839030-b132-4733-8815-da9a478e835b'", apiurls_constants.GetMatchLive+matchId)
	out, err := curl.Output()
	if err != nil {
		fmt.Println("erorr", err)
	}
